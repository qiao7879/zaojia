...
async def get_system_menu_list(
    request: Request,
    menu_query: Annotated[EnterpriseQueryModel, Query()],
    query_db: Annotated[AsyncSession, DBSessionDependency()]) -> Response:
    menu_query_result = await EnterpriseService.get_ent_list_services(query_db, menu_query)
    logger.info('获取成功')
...
async def delete_system_menu(
    ...
    delete_menu = DeleteEnterpriseModel(menuIds=ent_ids)
