...
class SysProjectPrefectOpinion(Base):
    __tablename__ = 'sys_project_prefect_opinion'
    __table_args__ = {
        'comment': '项目流程审核意见表',
    ...
    project_id = Column(BigInteger, nullable=False, ForeignKey('sys_project.id'), comment='关联项目ID')
    prefect_id = Column(BigInteger, nullable=False, ForeignKey('sys_project_prefect.id'), comment='关联流程ID（外键→sys_project_prefect.id）')
    node_code = Column(String(2), nullable=False, comment='意见对应的流程节点（01-创建/02-工程师修改/03-二级复审等）')
    node_name = Column(String(30), nullable=False, comment='节点名称（如二级复审）')
