...
from module_admin.entity.do.file_do import SysFile, FILE_STATUS_ENUM
...
class FileDao:
    ...
    async def get_file_by_id(cls, db: AsyncSession, file_id: int) -> Optional[SysFile]:
        result = (
            await db.execute(
                select(SysFile)
                .where(
                    and_(SysFile.id == file_id, SysFile.del_flag == '0', SysFile.status == FILE_STATUS_ENUM["NORMAL"]))
            )
        ).scalars().first()
        return result
    ...
    # 5. 分页查询文件
    @classmethod
    async def get_file_page_dao(
            cls, db: AsyncSession, query_params: FileQueryModel
    ) -> PageModel:
        query = (
            select(SysFile)
            .where(
        ...
        page_result = await PageUtil.paginate(
            db, query, query_params.page_num, query_params.page_size
        )
        return page_result
    ...
    async def delete_file_dao(cls, db: AsyncSession, file_id: int, update_user_id: int) -> None:
        await db.execute(
            ...
            .values(
                status=FILE_STATUS_ENUM["DELETE"],
